	...
	const uint8_t m)
{
	for (uint8_t row = row1; row <= row2; row++) {
		const int16_t y = m + row * FSZ.height;
		const int16_t x1 = m + (row == rs ? cs * FSZ.w : 0);
		const int16_t x2 = m + (row == re ? ce : CSZ.w) * FSZ.w;
...
static void change(const uint8_t m, SelEnd * se, SelEnd * ose)
{
	...
	invert(rs, re, cs, ce, row1, row2, m);
}
